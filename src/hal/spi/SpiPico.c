...
static PicoSpi *cast(Spi *self) { return (PicoSpi *)self; }
...
static inline uint8_t eai_to_pico_data_order(uint8_t data_order) { return SPI_MSB_FIRST; }
...
static void select(Spi *self, uint8_t slaveHandle) { gpioSetPin(slaveHandle, GPIO_PIN_LOW); }

static void deselect(Spi *self, uint8_t slaveHandle) { gpioSetPin(slaveHandle, GPIO_PIN_HIGH); }
