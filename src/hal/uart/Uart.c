...
void uartSetMqttReceiverFunction(void (*receive)(char *)) { uartMqttBrokerReceive = receive; }

void uartSetHTTPReceiverFunction(void (*receive)(char *)) { uartHTTPReceive = receive; }
...
bool uartCorrectResponseArrived(void) { return uartCorrectResponseReceived; }

void uartFreeCommandBuffer(void) { uartCommandToSend = "\0"; }
...
void checkAndHandleNewChar() {
  ...
  while (uart_is_readable((uart_inst_t *)uartConfig.uartInstance)) {
    ...
    if (!uartCurrentMessageIsHttpResponse && uartAwaitingHttpGet > 0 && receivedCharacter == ',') {
