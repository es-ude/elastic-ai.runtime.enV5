...
uint32_t crc32(const void *data, size_t len, uint32_t crc) {
    ...
    dma_channel_configure(sDMAChannel, &c, &dummy, data, len,
...
void copyPageInit(const void *src) {
    ...
    dma_channel_configure(sDMAChannel, &c, sPageBuffer, src, 256 / 4,
...
void flashFirmware(const tFlashHeader *header, uint32_t eraseLength) {
    ...
    while (pages > 0) {
        ...
        crc = copyPage();
        flash_range_program(flashoffset(sStart + offset), sPageBuffer, 256);
        offset += 256;
        pages--;
    }
    ...
    if (crc == crc32(&header->data[256], offset - 256, 0xffffffff)) {
        ...
        flash_range_program(flashoffset(sStart), sPageBuffer, 256);
...
void configClock(enum clock_index clk_index, uint32_t src, uint32_t auxsrc) {
    ...
    hw_write_masked(&clock->ctrl, src << CLOCKS_CLK_REF_CTRL_SRC_LSB, CLOCKS_CLK_REF_CTRL_SRC_BITS);
...
void initClock() {
    ...
    configClock(clk_ref, CLOCKS_CLK_REF_CTRL_SRC_VALUE_XOSC_CLKSRC,
                0); // No aux mux

    configClock(clk_sys, CLOCKS_CLK_SYS_CTRL_SRC_VALUE_CLKSRC_CLK_SYS_AUX,
...
int main(void) {
    ...
    while (image < (XIP_BASE + PICO_FLASH_SIZE_BYTES)) {
        ...
        watchdog_hw->scratch[3] = header->name[0];

        if ((header->magic1 == FLASH_MAGIC1) && (header->magic2 == FLASH_MAGIC2) &&
