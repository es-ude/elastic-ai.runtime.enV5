 ...
 *
 * Initializes UART connection, resets the ESP and checks if the connections is working until there is a response.
 * Then the command echo is  disabled and the ESP is set to single connection mode. The ChipStatus is set to
 * ESP_CHIP_OK.
 */
void ESP_Init(void);
 ...
 *
 * Send a command via UART to the ESP and waits the given amount of time for an expected answer.
 *
