...
void mqttBrokerConnectToBroker(char *brokerDomain, char *clientID) {
  ...
  espErrorCode_t error = espSendCommand(connectToBroker, AT_MQTT_CONNECT_TO_BROKER_RESPONSE, 60000);
  switch (error) {
  case ESP_NO_ERROR:
  ...
  case ESP_WRONG_ANSWER_RECEIVED:
    PRINT_DEBUG("Could not connect to %s at Port %s. Wrong answer!", mqttHost.ip, mqttHost.port);
...
char *communicationEndpointGetBrokerDomain() { return mqttBrokerBrokerId; }

char *communicationEndpointGetClientId() { return mqttBrokerClientId; }
