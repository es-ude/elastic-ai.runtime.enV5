 ...
 * until successful. Is successful if there is already any connection.
 *
 ...
 */
void MQTT_Broker_ConnectToBrokerUntilConnected(MQTTHost_t mqttHost,
                                               char *brokerDomain,
                                               char *clientID);

/*! \brief tries to connect to Broker
 *
 * Set the broker domain, client Id and then tries to connect to the host
 * broker. Is successful if there is already any connection.
 *
 * \param mqttHost contains ip and port of MQTT host
 * \param brokerDomain domain of broker, added before every message
 * \param clientID ID of this client, used to Identify to the Broker and added
 * after the Domain in every message \return true if connection successful or
 * already connected, otherwise false
 */
