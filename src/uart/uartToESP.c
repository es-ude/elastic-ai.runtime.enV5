...
void handleNewLine(void) {
    if (strlen(uartDev.receive_buf) != 0) {
        ...
        if (strncmp(command.expectedResponse, uartDev.receive_buf,
                    strlen(command.expectedResponse)) == 0) {
            command.responseArrived = true;
            strcpy(command.data,
                   &uartDev.receive_buf[strlen(command.expectedResponse)]);
        }
    }
}
...
bool uartToESP_ResponseArrived(void) { return command.responseArrived; }

void uartToESP_Free(void) { strcpy(command.cmd, "\0"); }
