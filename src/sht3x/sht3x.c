...
static sht3x_errorCode sendRequestToSensor(sht3x_command command) {
    ...
    sht3x_errorCode errorCode = I2C_WriteCommand(commandBuffer, sizeOfCommandBuffer,
                                                 sensorConfiguration.i2c_slave_address, sensorConfiguration.i2c_host);
...
static sht3x_errorCode receiveDataFromSensor(uint8_t *responseBuffer, uint8_t sizeOfResponseBuffer) {
    sht3x_errorCode errorCode = I2C_ReadData(responseBuffer, sizeOfResponseBuffer,
                                             sensorConfiguration.i2c_slave_address, sensorConfiguration.i2c_host);
...
static float calculateHumidity(uint16_t rawValue) {
    return 100.0f * ((float) rawValue / DENOMINATOR);
}
