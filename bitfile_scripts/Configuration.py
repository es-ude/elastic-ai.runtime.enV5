...
class Configuration:
    ...
    def is_headerdata(self, datasave):
        if len(datasave) > 16:
            if (np.array(datasave[-16:]) == 255).all(): return False
        return True
    ...
    def parse_header(self, header_data, header_info):
        ...
        if chr(header_data[design_descr_start]) != 'a':
            print('Bitfile header parsing error 1: design description starting bit sequence wrong')
            sys.exit(0)
        ...
        if chr(header_data[part_description_start]) != 'b':
            print('Bitfile header parsing error 2: part_description starting sequence wrong')
            sys.exit(0)
