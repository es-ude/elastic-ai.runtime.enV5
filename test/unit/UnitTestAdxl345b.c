...
i2cErrorCode_t i2cUnittestReadCommandPassInStreamMode(uint8_t *readBuffer, uint8_t sizeOfReadBuffer,
  ...
  if (sizeOfReadBuffer == 1) {
    readBuffer[0] =
        0b10000000 | 0b00000010 | 0b00011111; // set Stream Mode, Watermark and samples for Trigger
    return 0x00;
  }
...
i2cErrorCode_t i2cUnittestReadCommandPassInFifoMode(uint8_t *readBuffer, uint8_t sizeOfReadBuffer,
  ...
  if (sizeOfReadBuffer == 1) {
    readBuffer[0] =
        0b01000000 | 0b00000010 | 0b00011111; // set FiFo Mode, Watermark and samples for Trigger
    return 0x00;
  }
  i2cUnittestWriteByteMultipleTimes(readBuffer, sizeOfReadBuffer, byteZero);
  return 0x00;
...
i2cErrorCode_t i2cUnittestReadCommandPassInTriggerMode(uint8_t *readBuffer,
  ...
  if (sizeOfReadBuffer == 1) {
    readBuffer[0] =
        0b11000000 | 0b00000010 | 0b00011111; // set Trigger Mode, Watermark and samples for Trigger
    return 0x00;
  }
  i2cUnittestWriteByteMultipleTimes(readBuffer, sizeOfReadBuffer, byteZero);
  return 0x00;
...
adxl345bSensorConfiguration_t sensor = {
    .i2c_host = NULL,
    .i2c_slave_address = ADXL345B_I2C_ADDRESS,
    .full_res_mode = false,
    .measurement_range = ADXL345B_2G_RANGE,
};
...
void adxl345bGetSingleMeasurementReadCorrectValue(void) {
  ...
  TEST_ASSERT_EACH_EQUAL_UINT8(expectedRawData, rawData.rawData, 6);
  TEST_ASSERT_EQUAL_MESSAGE(sensor.measurement_range, rawData.measurement_range,
                            "Measurement Range not equal");
  TEST_ASSERT_EQUAL_MESSAGE(sensor.full_res_mode, rawData.full_res_mode, "Full Res Mode not equal");
}
...
void adxl345bGetMultipleMeasurementsGetSendCommandFail_errorIfHardwareFails(void) {
  ...
  adxl345bErrorCode_t errorCode = adxl345bGetMultipleMeasurements(sensor, samples, numberOfSamples);
  TEST_ASSERT_EQUAL_HEX8(ADXL345B_SEND_COMMAND_ERROR, errorCode);
}
...
/* region Convert Values Correct */

void adxl345bConvertDataXYZCorrectValueWithFullResolutionNegativeNumber(void) {
  /* test assumes 12bit with 8G Range*/
  float expected_xAxis = -1.2558f, expected_yAxis = -1.2558f, expected_zAxis = -1.2558f;

  adxl345bLsbSample_t lsbSample;
  adxl345bGValueSample_t gValueSample;

  adxl345bRawData_t testInput;
  testInput.full_res_mode = true;
  testInput.measurement_range = ADXL345B_8G_RANGE;
  memset(&testInput.rawData, 0xBE, sizeof(testInput.rawData));

  adxl345bErrorCode_t errorCode = adxl345bConvertBytesToLSB(testInput, &lsbSample);
  if (errorCode != ADXL345B_NO_ERROR) {
    printf("adxl345bConvertDataXYZ failed; adxl345b_ERROR: %02X", errorCode);
    TEST_FAIL();
  }
  ...
  TEST_ASSERT_EQUAL_FLOAT(expected_xAxis, gValueSample.x);
  TEST_ASSERT_EQUAL_FLOAT(expected_yAxis, gValueSample.y);
  TEST_ASSERT_EQUAL_FLOAT(expected_zAxis, gValueSample.z);
}

void adxl345bConvertDataXYZCorrectValueWithFullResolutionPositiveNumber(void) {
  /* test assumes 12bit with 8G Range*/
  float expected_xAxis = 6.7002f, expected_yAxis = 6.7002f, expected_zAxis = 6.7002f;
  ...
  testInput.full_res_mode = true;
  testInput.measurement_range = ADXL345B_8G_RANGE;
  memset(&testInput.rawData, 0xB6, sizeof(testInput.rawData));
...
void adxl345bConvertDataXYZCorrectValue10BitNegativeNumber(void) {
  /* test assumes 10bit with 8G Range*/
  float expected_xAxis = -5.0232f, expected_yAxis = -5.0232f, expected_zAxis = -5.0232f;

  adxl345bLsbSample_t lsbSample;
  adxl345bGValueSample_t gValueSample;

  adxl345bRawData_t testInput;
  testInput.full_res_mode = false;
  testInput.measurement_range = ADXL345B_8G_RANGE;
  memset(&testInput.rawData, 0xBE, sizeof(testInput.rawData));
...
void adxl345bConvertDataXYZCorrectValue10BitPositiveNumber(void) {
  /* test assumes 10bit with 8G Range*/
  float expected_xAxis = 2.9328f, expected_yAxis = 2.9328f, expected_zAxis = 2.9328f;
  ...
  testInput.full_res_mode = false;
  testInput.measurement_range = ADXL345B_8G_RANGE;
  memset(&testInput.rawData, 0xBC, sizeof(testInput.rawData));
...
void intConversion(void) {
  int16_t in1 = 45;
  TEST_ASSERT_EQUAL(0b00101101, (uint8_t)in1);

  int16_t in2 = -512;
  TEST_ASSERT_EQUAL(0xFF, (int8_t)in2);
}

int main(void) {
  UNITY_BEGIN();

  RUN_TEST(intConversion);
  ...
  RUN_TEST(adxl345bConvertDataXYZCorrectValueWithFullResolutionNegativeNumber);
  RUN_TEST(adxl345bConvertDataXYZCorrectValueWithFullResolutionPositiveNumber);
  RUN_TEST(adxl345bConvertDataXYZCorrectValue10BitNegativeNumber);
  RUN_TEST(adxl345bConvertDataXYZCorrectValue10BitPositiveNumber);
  return UNITY_END();
}
