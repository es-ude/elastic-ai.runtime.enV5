...
void unitTestInitParameterTensor() {
    float p[] = {1.f, 2.f, 3.f};
    size_t numberOfDimensions = 1;
    size_t dimensions[] = {3};

    parameterTensor_t *parameter = initParameterTensor(p, numberOfDimensions, dimensions);
    TEST_ASSERT_EQUAL_FLOAT_ARRAY(p, parameter->tensor->data, 3);

    float expectedGrad[] = {0.f, 0.f, 0.f};
    TEST_ASSERT_EQUAL_FLOAT_ARRAY(expectedGrad, parameter->grad, 3);

    TEST_ASSERT_EQUAL_size_t(3, calcTotalNumberOfElementsByTensor(parameter->tensor));
}
...
void testSequentialForwardWithConv1dAndCSV() {
    ...
    tensor_t *inputTensor = initTensor(csvInputFloats, inputNumberOfDims, inputDims);
...
void testSequentialBackwardUsingLinearAndCSV() {
    ...
    tensor_t *labelTensor = initTensor(csvLabelFloats, labelNumberOfDims, labelDims);
...
void test_Conv1d_ReLU_Linear_Forward() {
    ...
    tensor_t *inputTensor = initTensor(csvInputFloat, inputNumberOfDims, inputDims);
    ...
    parameterTensor_t *conv1dWeightTensor = initParameterTensor(
    ...
    parameterTensor_t *conv1dBiasTensor = initParameterTensor(
    ...
    parameterTensor_t *linearWeightTensor = initParameterTensor(
    ...
    parameterTensor_t *linearBiasTensor = initParameterTensor(
...
void test_Conv1d_ReLU_Linear_MSE_Backward() {
    ...
    tensor_t *labelTensor = initTensor(csvLabelFloat, labelNumberOfDims, labelDims);
...
void test_Conv1d_ReLU_Linear_Softmax_CrossEntropy() {
    ...
    tensor_t *distTensor = initTensor(csvDistributionFloat, distNumberOfDims, distDims);
...
int main() {
    UNITY_BEGIN();
    RUN_TEST(unitTestInitParameterTensor);
