...
void ADXL345B_ReadMeasurements_read_correct_value(void) {
    ...
    const uint8_t MSB_MASK               = 0x0003;
    const float   SCALE_FACTOR_FOR_RANGE = 0.0043f;
    
    /* fill expected with random generated */
    if (byteZero <= (MSB_MASK >> 1)) {
        /* CASE: positive value */
        int16_t rawValue  = (int16_t)(((uint16_t)(byteZero & MSB_MASK) << 8) | (uint16_t)byteZero);
         ...
         */
        uint16_t rawValue = ((uint16_t)(byteZero & (MSB_MASK >> 1)) << 8) | (uint16_t)byteZero;
    ...
    TEST_ASSERT_EQUAL_INT16(expected_xAxis, actual_xAxis);
    TEST_ASSERT_EQUAL_INT16(expected_yAxis, actual_yAxis);
    TEST_ASSERT_EQUAL_INT16(expected_zAxis, actual_zAxis);
  }
