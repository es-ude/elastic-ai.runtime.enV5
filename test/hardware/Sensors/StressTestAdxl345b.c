...
void init_adxl() {
    ...
    while (1) {
        ...
        // sollten wir setzen, da sonst per default 400000 gesetzt ist! das ist so aber nicht
        // schoen. wir koennten es auch dann einfach richtig initialisieren, wie wir es haben wollen
        // anstelle von eine "default init" und dann alles nochmal seperat einzugeben UND dann auch noch 400 falsch anstelle von 25?
        uint baudrate = i2c_set_baudrate(i2c1, 1600000);
        PRINT("baudrate is: %u", baudrate);

        errorCode =
            adxl345bWriteConfigurationToSensor(ADXL345B_REGISTER_BW_RATE, ADXL345B_BW_RATE_800);
        if (errorCode != ADXL345B_NO_ERROR) {
...
int main(void) {
    ...
    //vor der init schlafen lassen um sicher zu stellen, dass pico nicht zu schnell ist und adxl zu langsam nach reboot
    sleep_ms(1000);
    init_adxl();
