...
static void getAccelerometerData() {
    ...
    while (index <= LIMIT) {
        ...
        if (interrupt & BMI3_INT_STATUS_ACC_DRDY) {
            ...
            float x, y, z;
            x = bmi323LsbToMps2(data[0].sens_data.acc.x, BMI3_ACC_RANGE_16G, bmi323[0].resolution);
            y = bmi323LsbToMps2(data[0].sens_data.acc.y, BMI3_ACC_RANGE_16G, bmi323[0].resolution);
            z = bmi323LsbToMps2(data[0].sens_data.acc.z, BMI3_ACC_RANGE_16G, bmi323[0].resolution);
...
static void initializeGyroscope(void) {
    ...
    errorCode = bmi323PerformGyroscopeSelfCalibration(bmi323, BMI3_SC_OFFSET_EN,
                                                      BMI3_SC_APPLY_CORR_EN, &calibration);
    if (errorCode != BMI323_OK) {
        PRINT("\033[0;31mFAILED\033[0m; bmi323_ERROR: %02X", errorCode);
        return;
    }
...
static void getGyroscopeData() {
    ...
    while (index <= LIMIT) {
        ...
        float x, y, z;
        x = bmi323LsbToDps(data[0].sens_data.gyr.x, BMI3_GYR_RANGE_2000DPS, bmi323[0].resolution);
        y = bmi323LsbToDps(data[0].sens_data.gyr.y, BMI3_GYR_RANGE_2000DPS, bmi323[0].resolution);
        z = bmi323LsbToDps(data[0].sens_data.gyr.z, BMI3_GYR_RANGE_2000DPS, bmi323[0].resolution);
        PRINT("%d:\n"
              "\tRange=%dDPS\n"
              "\tRaw: X=0x%X, Y=0x%X, Z=0x%X\n"
