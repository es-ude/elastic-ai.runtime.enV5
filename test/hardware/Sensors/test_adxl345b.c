    ...
    if ( errorCode == ADXL345B_NO_ERROR )
      {
         ...
         * this deviation is given by the datasheet as the accepted tolerance for each axis
         * therefore should epsilon be 0.6G
         */
        float sumOfAxis = floatToAbs ( xAxis ) + floatToAbs ( yAxis ) + floatToAbs ( zAxis );
        printf ( compareFloatsWithinRange ( 1.0f, sumOfAxis, 0.6f ) ? "  \033[0;32mPASSED\033[0m; " : "  \033[0;31mFAILED\033[0m; " );
        printf ( "Expected: 01.0000G, Actual: %2.4fG = |%2.4fG| + |%2.4fG| + |%2.4fG| = X + Y + Z\n", sumOfAxis, xAxis, yAxis, zAxis );
      }
    else
      {
        printf ( "  \033[0;31mFAILED\033[0m; adxl345b_ERROR: %02X\n", errorCode );
...
int main ( void )
  {
    ...
    printf ( "Please enter to request g (G value), s (serialNo), t (self test), c (calibration) or b (Boot mode)\n" );
    while ( 1 )
      {
