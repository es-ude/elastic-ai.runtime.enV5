...
static adxl345bErrorCode_t changeTestedFifoMode(uint8_t *previousFifoMode, uint8_t testedFifoMode) {
    ...
    adxl345bErrorCode_t errorCode =
        adxl345bInternalReadDataFromSensor(sensor, ADXL345B_FIFO_CTL, previousFifoMode, 1);
...
static void checkInitValues(adxl345bRegister_t registerToCheck) {
    ...
    } else if (registerToCheck == ADXL345B_REGISTER_POWER_CTL) {
        expectedValue = 0b00001000;
    } else if (registerToCheck == ADXL345B_REGISTER_INT_ENABLE) {
...
parameterTest(checkInitValues, ADXL345B_REGISTER_POWER_CTL);
parameterTest(checkInitValues, ADXL345B_REGISTER_INT_ENABLE);
...
parameterTest(checkRangeValues, 2);
parameterTest(checkRangeValues, 4);
parameterTest(checkRangeValues, 8);
parameterTest(checkRangeValues, 16);
...
void setGivenFifoModeAndCheckValues(uint8_t testedFifoMode) {
    ...
    uint8_t buffer;
    errorCode = adxl345bInternalReadDataFromSensor(sensor, ADXL345B_FIFO_CTL, &buffer, 1);
    if (errorCode != ADXL345B_NO_ERROR) {
...
void checkSingleGValue(uint8_t testedFifoMode) {
    ...
    adxl345bLsbSample_t lsbData;
    errorCode = adxl345bConvertBytesToLSB(rawData, &lsbData);
    if (errorCode != ADXL345B_NO_ERROR) {
...
void checkMultipleGValues(uint8_t testedFifoMode) {
    ...
    if (testedFifoMode == ADXL345B_FIFOMODE_BYPASS) {
        ...
        if (errorCode != ADXL345B_NO_ERROR) {
            PRINT("calling adxl345bGetMultipleMeasurements in BYPASS-Mode should fail but does "
                  "not; adxl345b_ERROR: %02X",
                  errorCode);
            TEST_FAIL();
        } else {
            PRINT("calling adxl345GgetMultipleMeasurements in BYPASS-Mode should fail; "
                  "adxl345b_ERROR: %02X",
                  errorCode);
            TEST_FAIL();
        }
    ...
    errorCode = adxl345bConvertBytesToLSB(sensor, &xAxis, &yAxis, &zAxis, *rawData);
    if (errorCode != ADXL345B_NO_ERROR) {
        PRINT("adxl345bConvertDataXYZ failed; adxl345b_ERROR: %02X", errorCode);
        TEST_FAIL();
    }
...
void checkGValuesWithMeasuringForNMicroseconds(uint32_t microseconds) {
    ...
    errorCode =
        adxl345bGetMeasurementsForNMicroseconds(sensor, microseconds, rawData, numberOfSamples);
    if (errorCode != ADXL345B_NO_ERROR) {
    ...
    for (int i = 0; i < numberOfSamples * numberOfGValuesPerSample; i += numberOfGValuesPerSample) {
        errorCode = adxl345bConvertBytesToLSB(sensor, (gValues + i), (gValues + i + 1),
                                              (gValues + i + 2), *(rawData + i));
        if (errorCode != ADXL345B_NO_ERROR) {
            PRINT("adxl345bConvertDataXYZ failed; adxl345b_ERROR: %02X", errorCode);
            TEST_FAIL();
        }
    ...
    for (int i = 0; i < sizeOfGValues; i += numberOfGValuesPerSample) {
         ...
         * for each axis therefore should epsilon be 0.6G
         */
        sumsOfAxis[j] = floatToAbs(*(gValues + i)) + floatToAbs(*(gValues + i + 1)) +
                        floatToAbs(*(gValues + i + 2));
        j++;
