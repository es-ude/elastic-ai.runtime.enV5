...
class EnV5RecommendedRemoteControlProtocol(EnV5BaseRemoteControlProtocol):
    ...
    def _send_data_in_chunks(self, command: int, data: bytearray, chunk_size: int, ):
    ...
    def _receive_data_in_chunks(self, command: int, num_bytes: int, chunk_size: int) -> bytearray:
    ...
    def send_data_to_flash(self, flash_start_address: int, data: bytearray, chunk_size: int) -> bool:
        ...
        payload = bytearray()
        flash_start_address_raw = flash_start_address.to_bytes(length=4, byteorder=self.endian, signed=False)
        size_in_bytes = len(data)
        size_in_bytes_raw = size_in_bytes.to_bytes(length=4, byteorder=self.endian, signed=False)
    ...
    def read_data_from_flash(self, flash_start_address: int, num_bytes: int, chunk_size) -> bytearray:
        ...
        payload.extend(num_bytes.to_bytes(length=4, byteorder=self.endian, signed=False))
    ...
    def inference_with_data(self, chunk_size: int,
        ...
        inference_result = self._receive_data_in_chunks(command, num_bytes_outputs, chunk_size)
